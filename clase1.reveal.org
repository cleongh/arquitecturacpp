...
También se pueden usar Rider y VSCode, y cualquier otro editor más simple que pueda compilar y depurar
...
También tenemos un "atajo" para ~UE_LOG~, más cómodo y flexible: [[https://unrealcommunity.wiki/logging-lgpidy6i#logging-using-ue_logfmt][UE_LOGFMT]]
...
Para otros IDEs (Rider, VSCode, Xcode), el proceso es similar
...
#+caption: /Live coding/ funciona con casi todos los cambios de código, pero si cambiamos la estructura de clases, deberemos reiniciar el editor de UE
#+attr_html: :width 600 
...
Una vez creada la clase, vamos a instanciar un componente dentro de ella
...
=static= aquí hace que solo haya una instancia del assets compartida por todos los =Actor=, así /solo se carga la primera vez/
